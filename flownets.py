...
model = build_model("FlowNetS", default=True)

training_cfg = get_training_cfg(cfg_path="configs/flownets_trainer.yaml")
training_cfg.DEVICE = 0
training_cfg.DISTRIBUTED = False
training_cfg.VAL_INTERVAL = 1
training_cfg.LOG_DIR = "./logs/flownets/run1"
training_cfg.CKPT_DIR = "./ckpts/flownets/run1"

trainer = Trainer(training_cfg, model, train_loader, val_loader)
trainer.train(n_epochs=25)

# trainer = Trainer(training_cfg, model, train_loader, val_loader)
# trainer.resume_training("./ckpts/run6/flownets_epochs5.pth", n_epochs=5)
